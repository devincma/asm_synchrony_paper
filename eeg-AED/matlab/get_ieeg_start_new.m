function ieeg_start = get_ieeg_start_new(ipt)
    ...
    ieeg_start = zeros(size(ipt)); % Initialize the output array
    for i = 1:length(ipt)
        ptID =  ['HUP' num2str(ptIDs(ipt(i)))];
        [~,~,~,starts_eeg,starts_emu] = parse_MAR(ptID,all_meds);
        
        offsets = all_ieeg_offset{2,ipt(i)};
        ieeg_offset_datasets = all_ieeg_offset{1,ipt(i)};
        spike_rate = all_spike_rate{ipt(i)};
        time = all_spike_times{ipt(i)};
        
        % align ieeg times for each file with emu medication times
        offset_vec = file_inds{ipt(i)};
        for j = unique(offset_vec)'
            time_inds = (offset_vec==j);
            time(time_inds) = time(time_inds) - starts_eeg(j) + starts_emu(j);
        end
