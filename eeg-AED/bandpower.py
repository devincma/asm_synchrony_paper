...
band_names = ["delta", "theta", "alpha", "beta", "gamma", "high_gamma", "broad"]
...
for _, row in metadata.iterrows():
    ...
    for portal_name in iEEG_portal_names:
        ...
        # pull first 5 mins of every hour
        n_iter = int(np.floor(duration_min / 60))
        ...
        for i in tqdm(range(n_iter)):
            ...
            data, fs = tools.get_iEEG_data(
                ...
                start_usec,
                end_usec,
                select_electrodes=channel_labels,
            )
            ...
            sl_win = sliding_window_view(np.arange(n_samples), win_size_ind)[
                ::win_stride_ind, :
            ]
            ...
            for win_inds in sl_win:
                if (
                    np.sum(np.abs(signal_nan.iloc[win_inds, :]), axis=0) < 1 / 12
                ).any():
                    nan_mask[win_inds] = False
                if (
                    np.sqrt(np.sum(np.diff(signal_nan.iloc[win_inds, :]))) > 15000
                ).any():
                    nan_mask[win_inds] = False
            ...
            signal_filt = pd.DataFrame(
                signal_filt, columns=signal_nan.columns, index=signal_nan.index
            )
            ...
            for name, band in zip(band_names, bands):
                sl_win = sliding_window_view(
                    np.arange(signal_ref.shape[0]), win_size_ind
                )[::win_stride_ind, :]
                ...
                for ind, win_inds in enumerate(sl_win):
                    power_mat[ind, :] = tools.bandpower(
                        signal_ref.iloc[win_inds, :], fs, band, 1, relative=True
                    )
            ...
            col_names = [
                [
                    ...
                    for elec in signal_ref.columns
                ]
            ]
            indices = signal_ref.index[sl_win[:, -1]]
            bandpower_df = pd.DataFrame(
                all_bandpowers, index=indices, columns=col_names
            )
        ...
        band_powers.to_hdf(
            ospj(root_path, lb3_id, "ieeg_features", "{}.h5".format(portal_name)),
            key="bandpower",
            mode="w",
        )
